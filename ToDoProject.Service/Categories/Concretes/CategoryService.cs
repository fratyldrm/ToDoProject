...

{
    ...
    public async Task<ReturnModel<CategoryTodosDto>> GetCategoryWithToDosAsync(int categoryId)
    {
        var category = await categoryRepository.GetCategoryWithTodosAsync(categoryId);
        businessRules.CategoryExists(category);

        var categoryAsDto = mapper.Map<CategoryTodosDto>(category);

        return ReturnModel<CategoryTodosDto>.Success(categoryAsDto);
    }
    public async Task<ReturnModel<List<CategoryTodosDto>>> GetCategoryWithToDosAsync()
    {
        var category = await categoryRepository.GetCategoryWithTodos().ToListAsync();

        var categoryAsDto = mapper.Map<List<CategoryTodosDto>>(category);

        return ReturnModel<List<CategoryTodosDto>>.Success(categoryAsDto);
    }
